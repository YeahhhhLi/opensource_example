...
def parseFileHeader(header_bytes:bytes) -> int:
    if len(header_bytes) != kFileHeaderSize:
        print("Fail to parse FileHeader, invalid header size[%d]" % (len(header_bytes)))
        sys.exit(1)
    if header_bytes[:4] != kFileSignal:
        print("Fail to parse FileHeader, invalid header signal[%s]" % (header_bytes[:4]))
        sys.exit(1)
...
def parseAmsgHeader(amsgs_bytes:bytes) -> tuple:
    if len(amsgs_bytes) != kMsgHeaderSize:
        print("Fail to parse FileHeader, invalid header size[%d]" % (len(amsgs_bytes)))
        sys.exit(1)
    if amsgs_bytes[:4] != kMsgSignal:
        print("Fail to parse FileHeader, invalid header signal[%s]" % (amsgs_bytes[:4]))
        sys.exit(1)
...
def main(record_file:str):
    ...
    print("Succ to Parse TopicMsgDesc, topic_name[%s] message_type[%s]" 
        % (topic_msg_desc.topic_name, topic_msg_desc.message_type))
